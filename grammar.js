...
module.exports = grammar({
  ...
  supertypes: ($) => [$._prolog],
  rules: {
    module: ($) => seq(optional($.version_declaration), choice($.library_module, $.main_module)),
    ...
    _numeric_literal: ($) => choice($.integer_literal, $.decimal_literal, $.double_literal),
    ...
    parameter: ($) => seq(field('param_name', seq('$', $._EQName)), optional(field('param_type', $.type_declaration))),
    ...
    _postfix_expr: ($) => repeat1(choice($.predicate, $.postfix_lookup, $.arg_list)), // 49
    ...
    _relative_path_expr: ($) =>
      prec.left(field('step', seq($._step_expr, optional(repeat1(seq(choice('/', '//'), $._step_expr)))))), //109
    ...
    forward_axis: ($) =>
      seq(
        choice('child', 'descendant', 'attribute', 'self', 'descendant-or-self', 'following-sibling', 'following'),
        '::'
      ), //113
    ...
    multiplicative_expr: ($) =>
      prec.left(9, seq(field('lhs', $._expr), choice('*', 'div', 'idiv', 'mod'), field('rhs', $._expr))),
    union_expr: ($) => prec.left(10, seq(field('lhs', $._expr), choice('union', '|'), field('rhs', $._expr))),
    intersect_except_expr: ($) =>
      prec.left(11, seq(field('lhs', $._expr), choice('intersect', 'except'), field('rhs', $._expr))),
    instance_of_expr: ($) =>
      prec.left(12, seq(field('lhs', $._expr), seq('instance', 'of'), field('rhs', $.sequence_type))), // 92
    treat_expr: ($) => prec(13, seq(field('lhs', $._expr), seq('treat', 'as'), field('rhs', $.sequence_type))), // 93
    castable_expr: ($) => prec(14, seq(field('lhs', $._expr), seq('castable', 'as'), field('rhs', $.single_type))), // 94
    cast_expr: ($) => prec(15, seq(field('lhs', $._expr), seq('cast', 'as', field('rhs', $.single_type)))), // 95
    arrow_expr: ($) => prec.left(16, seq(field('rhs', $._expr), repeat1(seq('=>', $.arrow_function)))), // 94
    ...
    direct_element: ($) =>
      choice(seq($.start_tag, repeat(choice($._direct_constructor, $.element_text)), $.end_tag), $.empty_tag),
    ...
    direct_attribute: ($) =>
      seq(field('attr_name', alias($._QName, $.identifier)), '=', field('attr_value', $.direct_attribute_value)),
    ...
    _name_content: ($) =>
      seq(field('name', choice($._EQName, seq('{', commaSep($._expr), '}'))), field('content', $.enclosed_expr)),
    ...
    // 3.11.2 Arrays
    _array_constructor: ($) => choice($.curly_array_constructor, $.square_array_constructor), // 174 TODO ,
    ...
    let_binding: ($) => seq('$', field('var_name', $._EQName), optional($.type_declaration), ':=', $._expr),
    _intermediate_clause: ($) =>
      repeat1(choice($._initial_clause, $.where_clause, $.group_by_clause, $.order_by_clause, $.count_clause)), // 43',
    ...
    switch_expr: ($) =>
      prec(
        ...
        seq(
          ...
          field('switch_default', seq('default', 'return', field('default_return', $._expr)))
        )
      ), // 71
    switch_clause: ($) => seq('case', field('case_operand', $._expr), 'return', field('case_return', $._expr)), // 72
    ...
    quantified_expr: ($) =>
      prec(
        ...
        seq(
          ...
          optional($.type_declaration),
          'in',
          field('in_binding', seq($._expr, repeat(seq(',', $.var_ref, optional($.type_declaration), 'in', $._expr)))),
          'satisfies',
    ...
    typeswitch_expr: ($) =>
      prec(
        ...
        seq(
          ...
          field('default', seq('default', optional($.var_ref), 'return', $._expr))
        )
      ), // 74
    _typeswitch_clause: ($) =>
      seq(
    ...
    module_declaration: ($) =>
      seq('module', 'namespace', field('name', $.identifier), '=', field('uri', $.string_literal), ';'),
    ...
    copy_namespaces_declaration: ($) =>
      seq('declare', 'copy-namespaces', choice('preserve', 'no-preserve'), ',', choice('inherit', 'no-inherit')),
    // 4.10 Decimal Format Declaration
    decimal_format_declaration: ($) =>
      seq(
        'declare',
        choice(seq('decimal-format', field('name', $._EQName)), seq('default', 'decimal-format')),
    ...
    schema_import: ($) =>
      seq(
        ...
        optional(
          field(
            'prefix',
            choice(seq('namespace', field('name', $.identifier), '='), seq('default', 'element', 'namespace'))
          )
        ),
    ...
    namespace_declaration: ($) =>
      seq('declare', 'namespace', field('name', $.identifier), '=', field('uri', $.string_literal)), // 4.13
    default_namespace_declaration: ($) =>
      seq('declare', 'default', choice('element', 'function'), 'namespace', field('uri', $.string_literal)), // 4.14
    //4.15 Annotations
    annotation: ($) =>
      seq('%', field('name', $._EQName), field('body', optional(seq('(', commaSep($.string_literal), ')')))), // 27
    ...
    variable_declaration: ($) =>
      seq(
        ...
        optional($.type_declaration),
        choice(seq(':=', field('value', $._expr)), seq('external', optional(seq(':=', $._expr))))
      ), // 26
    ...
    context_item_declaration: ($) =>
      seq(
        ...
        'item',
        optional($.type_declaration),
        choice(seq(':=', field('var_value', $._expr)), seq('external', optional(seq(':=', $._expr))))
      ),
    ...
    option_declaration: ($) => seq('declare', 'option', field('name', $._EQName), field('value', $.string_literal)),
    ...
    element_test_params: ($) =>
      seq(
        ...
        optional(seq(',', field('param', seq($._EQName, optional(alias('?', $.occurrence_indicator))))))
      ),
    ...
    name_test: ($) => prec.left(choice($._EQName, $.wildcard)), // TODO 199
    ...
    _prefixed_name: ($) => seq(field('prefix', $.identifier), token.immediate(':'), field('local_part', $.identifier)),
